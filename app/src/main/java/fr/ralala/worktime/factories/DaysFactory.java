...
public class DaysFactory {
  ...
  private Map<Integer, List<DayEntry>> getWeeksAndDays(List<DayEntry> refList) {
    ...
    for (DayEntry de : refList) {
      ...
      list.computeIfAbsent(w, k -> new ArrayList<>());
      List<DayEntry> l = list.get(w);
      if (l != null)
        l.add(de);
    }
    return list;
   ...
   * @return DayEntry
   */
  public DayEntry getCurrentDay(Context ctx) {
    WorkTimeDay now = WorkTimeDay.now();
    DayEntry d = mSql.getDay(now.dateString());
    if (d == null)
      d = new DayEntry(ctx, WorkTimeDay.now(), DayType.ERROR, DayType.ERROR);
    return d;
  }
